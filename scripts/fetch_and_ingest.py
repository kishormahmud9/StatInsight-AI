...
def download_url_to_incoming(url: str, headers: Optional[Dict[str, str]] = None) -> Tuple[Optional[str], dict]:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            tmp_path = os.path.join(INCOMING_DIR, f".tmp_download_{int(time.time())}_{hashlib.md5(url.encode()).hexdigest()[:6]}")
            total = 0
            ...
            dest_path = os.path.join(INCOMING_DIR, dest_name)
            shutil.move(tmp_path, dest_path); tmp_path = None

            # dedupe by md5 — if identical content already present, replace the existing file
            # with the newly downloaded one, but first keep a timestamped backup of the previous file.
            try:
                ...
                for other in os.listdir(INCOMING_DIR):
                    ...
                    try:
                        if md5_of_file(op) == cur_md5:
                            ...
                            # move the newly downloaded file to replace the existing one
                            try:
                                shutil.move(dest_path, op)
                                log(f"Replaced existing file {op} with newly downloaded file for URL {url}")
                            except Exception:
                                ...
                                try:
                                    ...
                                    log(f"Failed to replace existing file {op}: {e}")
                                    # if replacement fails, keep the downloaded file for manual inspection
                                    status.update({"status": "failed_replace", "message": str(e), "path": dest_path})
                                    return dest_path, status
                        ...
                        # continue scanning other files even if one check fails
                        continue
            except Exception:
                # swallow errors here so download still succeeds if dedupe logic fails
                pass

            # dedupe by md5 (original behavior if not replacing)
            try:
                cur_md5 = md5_of_file(dest_path)
                for other in os.listdir(INCOMING_DIR):
                    if other == os.path.basename(dest_path):
                    ...
                    try:
                        if md5_of_file(op) == cur_md5:
                            os.remove(dest_path)
                            status.update({"status": "duplicate", "message": f"duplicate_of:{op}", "kept": op, "path": op})
...
def parse_args():
    ...
    p.add_argument("--dry", action="store_true", help="Dry-run mode: run fetch but run ingest in dry mode (or skip it)")
    return p.parse_args()
...
def main():
    ...
    try:
        ...
        return
    dry_ingest = args.dry
    log(f"Starting fetch_and_ingest (endpoints: {len(endpoints)}), dry_ingest={dry_ingest}")
